...
def predict_fn(input_data, model, max_seq_length):
    ...
    for jsonline in jsonlines:
        # features[0]:  news_text_body
        # features[1..n]:  is anything else (we can define the order ourselves)
        # Example:  
        #    {"features": ["This is good."]}        
        #
        news_text_body = json.loads(jsonline)["features"][0]

        input_ids, attention_mask = encode_review(news_text_body, max_seq_length)

        output = model(input_ids, attention_mask)

        # output is a tuple: 
        # output: (tensor([[ -0.9870,  2.8947]], grad_fn=<AddmmBackward>),
        # for torch.max() you need to pass in the tensor, output[0]   
        # _, prediction = torch.max(output[0], dim=1)
        prediction = output[0].squeeze()

        prediction = (prediction > 0.5).to(torch.long)

        

        prediction_dict = {}
        prediction_dict['predicted_label'] = prediction.item()
...
def process(args):
    ...
    df_test_reviews = pd.DataFrame(columns=['sentiment', 'news_text_body'])
    ...
    for file in input_files:
        ...
        df_temp = pd.read_csv(file_path,
                              sep='\t', 
                              usecols=['sentiment', 'news_text_body'])
    ...
    y_test = df_test_reviews['news_text_body'].map(predict)
