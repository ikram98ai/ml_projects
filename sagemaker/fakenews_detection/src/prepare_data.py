...
# list of sentiment classes: 0 - reliable; 1 - unreliable
classes = [0, 1]
...
def create_or_load_feature_group(prefix, feature_group_name):
    ...
    feature_definitions= [
        FeatureDefinition(feature_name='news_text_id', feature_type=FeatureTypeEnum.STRING),
        FeatureDefinition(feature_name='date', feature_type=FeatureTypeEnum.STRING),
        FeatureDefinition(feature_name='sentiment', feature_type=FeatureTypeEnum.STRING),
        FeatureDefinition(feature_name='label_id', feature_type=FeatureTypeEnum.STRING),
        FeatureDefinition(feature_name='input_ids', feature_type=FeatureTypeEnum.STRING),
        FeatureDefinition(feature_name='news_text_body', feature_type=FeatureTypeEnum.STRING),
    ...
    try:
        record_identifier_feature_name = "news_text_id"
...
################################################### Tokenization of the news_texts ################################################################


# Convert the news_text into the BERT input ids using 
def convert_to_bert_input_ids(news_text, max_seq_length):
...
def _preprocess_file(file, 
    ...
    # convert sentiment (0, 1) into label_id (0, 1)
    df['label_id'] = df['sentiment']
    print('df[label_id] after using classes_map: {}'.format(df['label_id']))

    df['news_body'] = "AUTHOR: " + str(df.author) + "NEWS_TITLE: " + str(df.title) + "NEWS_TEXT: " + str(df.text)

    df['news_body'] = df['news_body'].apply(clean_text)

    
    df['input_ids'] = df['news_body'].apply(lambda news_text: convert_to_bert_input_ids(news_text, max_seq_length))
    print('df[input_ids] after calling convert_to_bert_input_ids: {}'.format(df['input_ids']))   
    
    # convert the index into a news_text_id
    df.reset_index(inplace=True)
    df = df.rename(columns = {'index': 'news_text_id', 
                              'news_body': 'news_text_body'})
    
    # drop all columns except the following:
    df = df[['news_text_id', 'sentiment', 'label_id', 'input_ids', 'news_text_body']]
    df = df.reset_index(drop=True)
    ...
    column_names = ['news_text_id', 'sentiment', 'date', 'label_id', 'input_ids', 'news_text_body']
