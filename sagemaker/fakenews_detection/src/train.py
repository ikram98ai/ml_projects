...
def save_transformer_model(model:RobertaForSequenceClassification, model_dir):
    path = '{}/transformer'.format(model_dir)
    os.makedirs(path, exist_ok=True)                              
    print('Saving Transformer model to {}'.format(path))
    model.save_pretrained(path)

def save_pytorch_model(model:RobertaForSequenceClassification, model_dir):
...
# Create a custom Dataset class for the news
            ...
            if i < args.train_steps_per_epoch:
                ...
                if args.run_validation and i % args.validation_steps_per_epoch == 0:
                    ...
                    for sent, label in val_data_loader:
                        ...
                        output = model(sent)
                        prediction = output[0].squeeze()
                        predicted = (prediction > 0.5).to(torch.long)
                        total += label.size(0)
